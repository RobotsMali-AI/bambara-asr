...
- name: soloni-114m-tdt-ctc-v3
  ...
  - task:
    ...
    - name: Test WER
      type: wer
      value: 37.126072407980476
    - name: Test CER
      type: cer
      value: 21.17887541646023
  - task:
      name: Automatic Speech Recognition
      type: automatic-speech-recognition
    dataset:
...
`soloni-114m-tdt-ctc-v3` is a fine tuned version of [`RobotsMali/soloni-114m-tdt-ctc-v2`](https://huggingface.co/RobotsMali/soloni-114m-tdt-ctc-v2). The model was fine-tuned using **NVIDIA NeMo** and supports **both TDT (Token-and-Duration Transducer) and CTC (Connectionist Temporal Classification) decoding**.
...
asr_model = nemo_asr.models.EncDecHybridRNNTCTCBPEModel.from_pretrained(model_name="RobotsMali/soloni-114m-tdt-ctc-v3")
```
...
model_name = "RobotsMali/soloni-114m-tdt-ctc-v3"
...
The NeMo toolkit was used for finetuning this model for **13,388 steps** over `RobotsMali/soloni-114m-tdt-ctc-v2`. The finetuning codes and configurations can be found at [RobotsMali-AI/bambara-asr](https://github.com/RobotsMali-AI/bambara-asr/).
...
|---------------|----------|-----------------|-----------------|
| Kunkado | CTC | 37.16 | 19.75 |
| Nyana Eval    | CTC | 32.33 | 16.73 |
| Kunkado | TDT | 37.12 | 21.17 |
| Nyana Eval    | TDT | 35.32 | 18.22 |
